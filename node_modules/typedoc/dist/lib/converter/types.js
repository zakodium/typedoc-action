...
const typeLiteralConverter = {
    ...
    convert(context, node) {
        ...
        for (const signature of type.getCallSignatures()) {
            ...
            reflection.signatures.push(signature_1.createSignature(rc, models_1.ReflectionKind.CallSignature, signature));
        }
    ...
    convertType(context, type) {
        ...
        for (const signature of type.getCallSignatures()) {
            (_a = reflection.signatures) !== null && _a !== void 0 ? _a : (reflection.signatures = []);
            reflection.signatures.push(signature_1.createSignature(context.withScope(reflection), models_1.ReflectionKind.CallSignature, signature));
        }
...
const referenceConverter = {
    ...
    convert(context, node) {
        ...
        type.typeArguments = (_a = node.typeArguments) === null || _a === void 0 ? void 0 : _a.map((type) => convertType(context, type));
        return type;
    },
    convertType(context, type) {
        var _a, _b;
        ...
        ref.typeArguments = (_b = (type.aliasSymbol
            ? type.aliasTypeArguments
            : type.typeArguments)) === null || _b === void 0 ? void 0 : _b.map((ref) => convertType(context, ref));
        return ref;
    },
};
...
const tupleConverter = {
    ...
    convert(context, node) {
        var _a;
        // TS 3.9 support
        const elementTypes = (_a = node.elements) !== null && _a !== void 0 ? _a : node.elementTypes;
        const elements = elementTypes.map((node) => convertType(context, node));
        return new models_1.TupleType(elements);
    },
    convertType(context, type, node) {
        var _a;
        let elements = (_a = type.typeArguments) === null || _a === void 0 ? void 0 : _a.map((type) => convertType(context, type));
        ...
        if (node.elements && node.elements.every(ts.isNamedTupleMember)) {
            const namedMembers = node.elements;
            elements = elements === null || elements === void 0 ? void 0 : elements.map((el, i) => new models_1.NamedTupleMember(namedMembers[i].name.text, !!namedMembers[i].questionToken, reflections_1.removeUndefined(el)));
        }
...
const typeOperatorConverter = {
    ...
    convertType(context, type, node) {
        ...
        if (node.operator === ts.SyntaxKind.ReadonlyKeyword) {
            ...
            const inner = type.objectFlags & ts.ObjectFlags.Tuple
