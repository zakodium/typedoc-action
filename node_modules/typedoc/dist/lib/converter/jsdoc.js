...
exports.convertJsDocCallback = exports.convertJsDocTypedef = void 0;
...
function convertJsDocTypedef(context, symbol, declaration, nameOverride) {
    var _a;
...
exports.convertJsDocTypedef = convertJsDocTypedef;
function convertJsDocCallback(context, symbol, declaration, nameOverride) {
    const alias = context.createDeclarationReflection(models_1.ReflectionKind.TypeAlias, symbol, nameOverride);
...
function convertJsDocInterface(context, declaration, symbol, nameOverride) {
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Interface, symbol, nameOverride);
...
function convertTemplateParameters(context, node) {
    var _a;
    assert_1.ok(context.scope instanceof models_1.DeclarationReflection);
...
function convertTemplateParameterNodes(context, nodes) {
    const params = array_1.flatMap(nodes !== null && nodes !== void 0 ? nodes : [], (tag) => tag.typeParameters);
    return signature_1.convertTypeParameterNodes(context, params);
}
