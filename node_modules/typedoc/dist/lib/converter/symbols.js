...
function getSymbolExportsWithFlag(symbol, flag) {
    var _a;
    const childSymbols = [];
    (_a = symbol.exports) === null || _a === void 0 ? void 0 : _a.forEach((child) => {
        if (child.flags & flag) {
...
function convertSymbol(context, symbol, nameOverride) {
    var _a;
    ...
    if (enum_1.hasFlag(symbol.flags, ts.SymbolFlags.Class)) {
        flags = enum_1.removeFlag(flags, ts.SymbolFlags.Interface);
    }
    // Kind of declaration merging... we treat this as a property with get/set signatures.
    if (enum_1.hasFlag(symbol.flags, ts.SymbolFlags.GetAccessor)) {
        flags = enum_1.removeFlag(flags, ts.SymbolFlags.SetAccessor);
    }
    if (enum_1.hasFlag(symbol.flags, ts.SymbolFlags.NamespaceModule)) {
        // This might be here if a namespace is declared several times.
        flags = enum_1.removeFlag(flags, ts.SymbolFlags.ValueModule);
    }
    if (enum_1.hasFlag(symbol.flags, ts.SymbolFlags.Method)) {
    ...
    for (const flag of enum_1.getEnumFlags(flags)) {
        ...
        (_a = symbolConverters[flag]) === null || _a === void 0 ? void 0 : _a.call(symbolConverters, context, symbol, nameOverride);
    }
}
...
function convertEnum(context, symbol, nameOverride) {
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Enum, symbol, nameOverride);
    if (symbol.flags & ts.SymbolFlags.ConstEnum) {
        reflection.setFlag(models_1.ReflectionFlag.Const);
    }
    convertSymbols(context.withScope(reflection), getSymbolExportsWithFlag(symbol, ts.SymbolFlags.EnumMember));
}
function convertEnumMember(context, symbol, nameOverride) {
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.EnumMember, symbol, nameOverride);
    reflection.defaultValue = JSON.stringify(context.checker.getConstantValue(symbol.getDeclarations()[0]));
}
function convertNamespace(context, symbol, nameOverride) {
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Namespace, symbol, nameOverride);
    convertSymbols(context.withScope(reflection), getSymbolExportsWithFlag(symbol, ts.SymbolFlags.ModuleMember));
}
function convertTypeAlias(context, symbol, nameOverride) {
    var _a, _b;
    ...
    if (ts.isTypeAliasDeclaration(declaration)) {
        const reflection = context.createDeclarationReflection(models_1.ReflectionKind.TypeAlias, symbol, nameOverride);
        ...
        jsdoc_1.convertJsDocTypedef(context, symbol, declaration, nameOverride);
    }
    else {
        jsdoc_1.convertJsDocCallback(context, symbol, declaration, nameOverride);
    }
}
...
function convertFunctionOrMethod(context, symbol, nameOverride) {
    ...
    if (isMethod &&
        ...
        enum_1.hasFlag(ts.getCombinedModifierFlags(declarations[0]), ts.ModifierFlags.Private)) {
        return;
    }
    ...
    const reflection = context.createDeclarationReflection(isMethod ? models_1.ReflectionKind.Method : models_1.ReflectionKind.Function, symbol, nameOverride);
    ...
    for (const [signature, declaration] of array_1.zip(signatures, declarations)) {
        const converted = signature_1.createSignature(scope, models_1.ReflectionKind.CallSignature, signature, declaration);
...
function convertClassOrInterface(context, symbol, nameOverride) {
    ...
    const classDeclaration = (_a = symbol
        .getDeclarations()) === null || _a === void 0 ? void 0 : _a.find(ts.isClassDeclaration);
    if (classDeclaration) {
...
function convertProperty(context, symbol, nameOverride) {
    var _a, _b, _c, _d;
    ...
    if (declarations.length === 1) {
        ...
        if (ts.isPropertyDeclaration(declaration) &&
            ...
            return convertArrowAsMethod(context, symbol, declaration.initializer, nameOverride);
        }
    }
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Property, symbol, nameOverride);
...
function convertArrowAsMethod(context, symbol, arrow, nameOverride) {
    var _a;
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Method, symbol, nameOverride);
...
function convertConstructor(context, symbol) {
    var _a, _b;
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Constructor, symbol, "constructor");
    const reflectionContext = context.withScope(reflection);
...
function convertAlias(context, symbol, nameOverride) {
    ...
    if (!reflection) {
        ...
        convertSymbol(context, context.resolveAliasedSymbol(symbol), nameOverride !== null && nameOverride !== void 0 ? nameOverride : symbol.name);
    }
    else {
        // We already have this. Create a reference.
        const ref = new models_1.ReferenceReflection(nameOverride !== null && nameOverride !== void 0 ? nameOverride : symbol.name, reflection, context.scope);
...
function convertVariable(context, symbol, nameOverride) {
    var _a;
    ...
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Variable, symbol, nameOverride);
    let typeNode;
    ...
    if (enum_1.hasFlag(symbol.flags, ts.SymbolFlags.BlockScopedVariable)) {
        reflection.setFlag(models_1.ReflectionFlag.Const, enum_1.hasFlag(declaration.parent.flags, ts.NodeFlags.Const));
    }
    reflection.defaultValue = convert_expression_1.convertDefaultValue(declaration);
}
function convertVariableAsFunction(context, symbol, nameOverride) {
    var _a, _b;
    const declaration = (_a = symbol
        .getDeclarations()) === null || _a === void 0 ? void 0 : _a.find(ts.isVariableDeclaration);
    assert(declaration);
    const type = context.checker.getTypeOfSymbolAtLocation(symbol, declaration);
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Function, symbol, nameOverride);
    setModifiers(declaration, reflection);
    // Does anyone care about this? I doubt it...
    if (enum_1.hasFlag(symbol.flags, ts.SymbolFlags.BlockScopedVariable)) {
        reflection.setFlag(models_1.ReflectionFlag.Const, enum_1.hasFlag(declaration.parent.flags, ts.NodeFlags.Const));
...
function convertAccessor(context, symbol, nameOverride) {
    var _a, _b;
    const reflection = context.createDeclarationReflection(models_1.ReflectionKind.Accessor, symbol, nameOverride);
    const rc = context.withScope(reflection);
    const getDeclaration = (_a = symbol.getDeclarations()) === null || _a === void 0 ? void 0 : _a.find(ts.isGetAccessor);
    if (getDeclaration) {
    ...
    const setDeclaration = (_b = symbol.getDeclarations()) === null || _b === void 0 ? void 0 : _b.find(ts.isSetAccessor);
    if (setDeclaration) {
...
function setModifiers(declaration, reflection) {
    ...
    if (enum_1.hasFlag(modifiers, ts.ModifierFlags.Private)) {
        reflection.setFlag(models_1.ReflectionFlag.Private);
    }
    if (enum_1.hasFlag(modifiers, ts.ModifierFlags.Protected)) {
        reflection.setFlag(models_1.ReflectionFlag.Protected);
    }
    if (enum_1.hasFlag(modifiers, ts.ModifierFlags.Public)) {
        reflection.setFlag(models_1.ReflectionFlag.Public);
    }
    reflection.setFlag(models_1.ReflectionFlag.Optional, !!declaration.questionToken);
    reflection.setFlag(models_1.ReflectionFlag.Readonly, enum_1.hasFlag(modifiers, ts.ModifierFlags.Readonly));
    reflection.setFlag(models_1.ReflectionFlag.Abstract, enum_1.hasFlag(modifiers, ts.ModifierFlags.Abstract));
    reflection.setFlag(models_1.ReflectionFlag.Static, enum_1.hasFlag(modifiers, ts.ModifierFlags.Static));
}
