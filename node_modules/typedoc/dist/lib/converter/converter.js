...
let Converter = Converter_1 = class Converter extends component_1.ChildableComponent {
    ...
    convertExports(context, node, entryPoints, entryName) {
        ...
        if (entryPoints.length === 1) {
            ...
            const reflection = context.createDeclarationReflection(index_1.ReflectionKind.Module, symbol, entryName);
            moduleContext = context.withScope(reflection);
        }
        for (const exp of getExports(context, node).filter((exp) => isDirectExport(context.resolveAliasedSymbol(exp), node))) {
            symbols_1.convertSymbol(moduleContext, exp);
        }
        return moduleContext;
    }
    convertReExports(moduleContext, node) {
        for (const exp of getExports(moduleContext, node).filter((exp) => !isDirectExport(moduleContext.resolveAliasedSymbol(exp), node))) {
            symbols_1.convertSymbol(moduleContext, exp);
        }
    }
    /**
    ...
    shouldIgnore(symbol, checker) {
        if (this.excludeNotDocumented &&
            ...
            !enum_1.hasFlag(symbol.flags, ts.SymbolFlags.EnumMember) &&
...
function getExports(context, node) {
    var _a, _b, _c;
    let symbol = (_a = context.checker.getSymbolAtLocation(node)) !== null && _a !== void 0 ? _a : node.symbol;
    ...
    const exportEq = (_b = symbol === null || symbol === void 0 ? void 0 : symbol.exports) === null || _b === void 0 ? void 0 : _b.get("export=");
    if (exportEq) {
        return [exportEq];
    }
    if (symbol) {
    ...
    if (globalSymbols.length === 1 && ((_c = globalSymbols[0]
        .getDeclarations()) === null || _c === void 0 ? void 0 : _c.every((declaration) => ts.isModuleDeclaration(declaration) &&
